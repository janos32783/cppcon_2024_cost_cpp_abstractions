...
concept IPin = requires (T pin) {
    { pin.get_pin_number() } -> std::same_as<std::uint8_t>;
};
...
private:
    std::uint8_t m_pin;
public:
    CPinA (std::uint8_t pin) : m_pin(pin) {}
    std::uint8_t get_pin_number () const { return m_pin + 1; }
};
...
private:
    std::uint8_t m_pin;
public:
    CPinB (std::uint8_t pin) : m_pin(pin) {}
    std::uint8_t get_pin_number () const { return m_pin - 1; }
};

template <IPin TPin>
std::uint8_t HandlePin (TPin* pin) {
...
int main (void) {
    ...
    volatile std::uint8_t pin_number;

    CPinA pin_a { GPIO_PIN_6 };
    pin_number = HandlePin<CPinA>(&pin_a);
    CPinB pin_b { GPIO_PIN_6 };
    pin_number = HandlePin<CPinB>(&pin_b);
